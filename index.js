...
let upperHacker1 = separateHacker1
  .toUpperCase()
  .slice(0, separateHacker1.length - 1);
...
function phraseToCheck(phrase) {
  ...
  for (let i = 0; i <= phrase.length - 1; i++) {
    if (phrase[i] !== " " && phrase[i] !== "," && phrase[i] !== "!") {
      noSpacePhrase += phrase[i];
    }
  }
  console.log(noSpacePhrase);
  let reversedNoSpace = "";
  for (let i = noSpacePhrase.length - 1; i >= 0; i--) {
    reversedNoSpace += noSpacePhrase[i];
  }
  if (noSpacePhrase.toUpperCase() === reversedNoSpace.toUpperCase()) {
